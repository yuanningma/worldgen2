...
The continent initializer now separates initial crustal communities before clustering subordinate terranes, increases the subordinate lobe hierarchy, and gives competing fronts 20% aggregate reserve capacity. This removes the mathematical requirement for the union-front fallback to inflate every overlap into a smooth hull. It does not specify a final continent count: 120 Myr of transport, rifting, collision, erosion, and flooding still determine the emerged components. The `primeval-atlas-7` coupled morphology gate now passes with four major lands, maximum major elongation 1.47, ribbon severity 0.94, and coastline richness 0.45 at the subdivision-4 evaluation tier.

Resolved river topology remains the conservative Priority-Flood graph, but presentation centerlines no longer connect lightly displaced face centers. Five basin-spine relaxation passes produce shared junction nodes, two-step look-ahead tangents keep dominant channels continuously differentiable through process-cell turns, and seven to eleven within-edge samples carry coherent lowland bends. At the subdivision-6 reference tier, mean sinuosity rises from 1.024 to about 1.037 while receiver IDs, lake membership, mouths, annual runoff, and sediment budgets remain unchanged by the centerline construction. Higher hydrology resolution and true sub-cell erosion remain later core work; the current change removes much of the visible triangular-grid direction without pretending that rendering created new drainage.
