...
5. **Completed structural baseline; held-out calibration remains:** the initializer now creates 5-7 uneven primordial crustal communities and 9-13 subordinate terrane nuclei. Terranes within one community can suture, while competition between communities preserves ocean corridors. Coupled history, breakup, and flooding still decide emerged landmasses; there is no final continent-count parameter. Continue calibrating seeds that still form supercontinents, zonal ribbons, or over-elongated systems before adding renderer detail.
