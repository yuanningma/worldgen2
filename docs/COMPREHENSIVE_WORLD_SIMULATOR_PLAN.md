...
**Status: annual physical-atlas milestone implemented; hierarchical drainage and orogenic calibration underway.** Reduced spherical prevailing winds, physical-distance-scaled moisture advection, orographic loss, land distance-to-ocean, continentality, seasonal-temperature range, PET-relative aridity, runoff closure, Priority-Flood routing, bounded spill-basin lake cover, river hierarchy, smoothed river presentation, stream-power incision, lithology resistance, biome classification, and closed sediment/area budgets exist. Atmospheric pass count, recharge, precipitation loss, recycling, and uplift response are expressed against physical edge distance instead of process-cell count. Surface relief now distinguishes narrow continental-collision cores, landward subduction ranges, oceanic island arcs, inherited sutures, and broader foothills. Unsupported high elevation is compressed without changing canonical land authority, and an orogeny diagnostic exposes the geological support behind every rendered range. Seasonal pressure/circulation, hierarchical basin anchoring, process-grid channel refinement, hillslope diffusion, explicit lake water balance, deltas, and empirical biome calibration remain open.
...
The high-resolution review gate now requires a 4096×2048 whole-world atlas plus native-pixel regional crops generated by `npm run review:surface-atlas`. Raster enlargement alone remains insufficient because process-mesh facets become more visible even when coastline pixels are stable. Raising the process grid from subdivision 6 to 7 quadruples cells from 81,920 to 327,680. Physical-distance atmosphere scaling now makes three fixed-history seed comparisons pass: precipitation and runoff drift stay below 1%, arid-land drift below 1.3 percentage points, lake-area drift below 17%, and biome total variation below 1%. The actual coupled-render path likewise converges for climate, runoff, lakes, and biomes, but its largest drainage basin changes by 31.5% because refinement changes which outlet captures a major saddle. Discharge and total runoff remain stable, so this is a drainage-topology problem rather than a water-budget problem. The next gate is a stable coarse basin graph with fine channels constrained to inherit its outlets; renderer smoothing must not conceal this failure.
...
1. Anchor fine-grid Priority-Flood outlets and basin membership to a stable coarser drainage graph, then refine channels inside each inherited basin without changing continental divides.
2. Re-run subdivision-6/7 convergence for the coupled history and require stable drainage hierarchy in addition to the already-converged temperature, precipitation, runoff, lakes, and biome area.
