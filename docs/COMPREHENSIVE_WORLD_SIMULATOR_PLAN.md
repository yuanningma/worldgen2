...
**Status: annual atlas diagnostics and initial presentation baseline implemented.** Natural, heightmap, climate, biomes, precipitation, aridity, temperature, continentality, drainage, wind, and lithology modes share one canonical world in offline renders. The natural renderer now has an illustrated atlas style, chosen as the default for aesthetic iteration, and a restrained physical-relief style. The atlas view uses flat water and explicit coast ink instead of an accidental luminous shelf; the relief view keeps bathymetry and local shading but confines the shelf ramp to a narrow, dark band. Both consume identical canonical geography and process fields. Adaptive coast-only supersampling reduces raster stair steps without globally multiplying render cost. The new simulator is not deployed or wired into the legacy studio; retained-model browser switching and progressive tiled rendering remain open.

The high-resolution review gate now requires a 4096×2048 whole-world atlas plus native-pixel regional crops generated by `npm run review:surface-atlas`. The first gate run showed that raster enlargement alone is useful but insufficient: coastline pixels remain stable, while broad orogenic ribbons and process-mesh facets become more obvious. Raising the process grid from subdivision 6 to 7 quadruples cells from 81,920 to 327,680 and increases the resolved river graph from 3,022 to 6,250 segments. `npm run evaluate:surface-resolution` currently rejects that transition: mean precipitation drifts 13.5%, total runoff 16.3%, arid-land fraction 16.2 percentage points, and derived lake area 75.6%, although land fraction, mean temperature, and total biome distribution are already stable. Resolution convergence, retained-model caching, and multiscale relief therefore precede seasonal climate and ocean heat transport.
...
3. Replace broad ribbon relief with collision/subduction-specific multiscale profiles, foothills, and local-relief shading on the retained process grid.
