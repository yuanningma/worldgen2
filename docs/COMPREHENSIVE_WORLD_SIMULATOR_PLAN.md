...
**Status: annual physical-atlas and hierarchical-drainage milestone implemented; local channel geometry and orogenic calibration underway.** Reduced spherical prevailing winds, physical-distance-scaled moisture advection, orographic loss, land distance-to-ocean, continentality, seasonal-temperature range, PET-relative aridity, runoff closure, hierarchical Priority-Flood routing, bounded spill-basin lake cover, river hierarchy, smoothed river presentation, stream-power incision, lithology resistance, biome classification, and closed sediment/area budgets exist. Atmospheric pass count, recharge, precipitation loss, recycling, and uplift response are expressed against physical edge distance instead of process-cell count. A deterministic one-level-coarser process world now owns inland basin succession; fine descendants may refine local channels and coastal outlets but every inland cross-parent receiver must inherit its anchor receiver. Surface relief distinguishes narrow continental-collision cores, landward subduction ranges, oceanic island arcs, inherited sutures, and broader foothills. Unsupported high elevation is compressed without changing canonical land authority, and an orogeny diagnostic exposes the geological support behind every rendered range. Seasonal pressure/circulation, sub-cell channel geometry, hillslope diffusion, explicit lake water balance, deltas, and empirical biome calibration remain open.
...
The high-resolution review gate now requires a 4096×2048 whole-world atlas plus native-pixel regional crops generated by `npm run review:surface-atlas`. Raster enlargement alone remains insufficient because process-mesh facets become more visible even when coastline pixels are stable. Raising the process grid from subdivision 6 to 7 quadruples cells from 81,920 to 327,680. Physical-distance atmosphere scaling makes three fixed-history seed comparisons pass: precipitation and runoff drift stay below 1%, arid-land drift below 1.3 percentage points, lake-area drift below 14%, and biome total variation below 1%. Hierarchical drainage reduces their major-basin drift to 0.015–0.066%. The actual coupled-render path now passes every current convergence gate: its major-basin drift falls from 31.5% to floating-point noise, lake-area drift is 7.7%, and all non-coastal fine receiver transitions agree with the anchor graph. Water and sediment budgets still close. Diagnostic renders show that exact river and lake cells remain visibly triangular; that local geometry must be reconstructed below the basin graph rather than altering stable divides or hiding them with raster blur.
...
1. Reconstruct sub-cell river centerlines and lake surfaces inside the stable inherited basin graph; preserve receiver order, discharge, lake volume, and ocean outlets while removing visible triangular facets.
2. Add explicit annual lake water balance, evaporation, overflow, and connected shore contours instead of interpreting Priority-Flood capacity as lake volume.
