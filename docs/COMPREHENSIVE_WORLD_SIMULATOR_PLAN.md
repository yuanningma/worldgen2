...
**Status: annual physical-atlas milestone implemented; orogenic calibration underway.** Reduced spherical prevailing winds, moisture advection, orographic loss, land distance-to-ocean, continentality, seasonal-temperature range, PET-relative aridity, runoff closure, Priority-Flood routing, deep-basin lake cover, river hierarchy, smoothed river presentation, stream-power incision, lithology resistance, biome classification, and closed sediment/area budgets exist. Surface relief now distinguishes narrow continental-collision cores, landward subduction ranges, oceanic island arcs, inherited sutures, and broader foothills. Unsupported high elevation is compressed without changing canonical land authority, and an orogeny diagnostic exposes the geological support behind every rendered range. Seasonal pressure/circulation, process-grid channel refinement, hillslope diffusion, explicit lake evolution, deltas, empirical biome calibration, and subdivision-6/7 convergence remain open.
...
The high-resolution review gate now requires a 4096×2048 whole-world atlas plus native-pixel regional crops generated by `npm run review:surface-atlas`. The first gate run showed that raster enlargement alone is useful but insufficient: coastline pixels remain stable, while process-mesh facets become more obvious. Raising the process grid from subdivision 6 to 7 quadruples cells from 81,920 to 327,680. After boundary-specific orogenic shaping, `npm run evaluate:surface-resolution` reports stable land fraction, mean temperature, drainage maximum, total biome area, and a lake-area drift reduced from 75.6% to 40.7%, inside its current bound. The transition still fails total-runoff drift at 16.3% against a 15% limit and arid-land drift at 16.0 percentage points against an 8-point limit; mean precipitation drift remains 13.5%. Resolution convergence and retained-model caching therefore still precede seasonal climate and ocean heat transport.
...
1. Improve canonical surface elevation, bathymetry, coast antialiasing, and drainage density without changing macro-land authority; judge silhouette in the illustrated atlas view and physical plausibility in the relief view.
2. Establish subdivision-6/7 convergence bounds for temperature, precipitation, runoff, drainage hierarchy, lakes, and biome area before adding seasonal forcing.
3. Calibrate the implemented collision/subduction/island-arc/suture profiles across accepted seeds, then replace remaining process-grid facets with convergent multiscale local-relief shading.
4. Calibrate the implemented land distance-to-ocean, continentality, aridity, and biome fields against Earth-derived ranges and diverse accepted seeds without allowing classification to mutate climate or coast state.
5. Retain one simulated surface in a worker and switch diagnostic modes, atlas/globe projection, and style without regeneration; progressively rasterize 4K–8K tiles from that retained state.
6. Calibrate whole-world composition and resolved detail against Earth epochs, synthetic fixtures, the Malazan-inspired private aesthetic targets, and the catalogued external simulator references.
