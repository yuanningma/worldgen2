...
The coastline-coherence milestone now makes the displaced signed shoreline a shared presentation boundary instead of a renderer-only mask. Ocean-bound channels are bisected from their final land node toward the canonical water receiver until they lie on that zero-contour; delta and fan branches independently resolve their endpoints against the same coast. A public audit records boundary mismatches and maximum residual, with a three-seed gate requiring every ocean mouth to remain within 10 m of the refined coast. The 4096×2048 coupled review resolves 143 ocean mouths with zero mismatches and a numerical maximum residual below `1e-6 km`. Process coastal cells now carry a measured center-to-boundary distance instead of zero, while categorical relief and coast fields use wider continuous elevation support plus exact spherical point-to-segment distance to nearby coast segments. Coast-specific ruggedness and sediment affinity taper to zero before leaving their local canonical support, removing triangular shelf/highland halos without altering topology anchors. A retained tile/cache is required before this segment-distance field is sampled interactively at 8K; this is a browser-rendering optimization, not a reason to lower final world resolution.

The first continuous-terrain milestone now lifts collision, subduction, island-arc, and inherited-suture source edges onto the nested surface mesh before calculating relief. Four land-only physical-distance fields create finite-width cores, foothills, foreland basins, and outer flexural rises instead of assigning one orogenic profile to every descendant of a canonical face. Along-strike source modulation breaks uniform range sheets while remaining deterministic and causally tied to the tectonic boundary network. Three fixed-seed subdivision-3→4 gates place 26.4–32.2% of land cells inside the deliberately broad prototype core threshold, keep the maximum adjacent land strength jump to 0.234–0.390, preserve every coast anchor, and confirm meaningful relief variation inside individual canonical faces. Fine sampled relief now uses a three-direction domain-warped spectrum at roughly 14–230 km wavelengths. Those octaves affect height and restrained albedo, but remain absent from analytical hillshade until an output-scale tiled normal-map pass can prevent directional aliasing. The climate-scale terrain filter now subtracts the mesh cell's existing footprint before adding an unresolved 220 km kernel, and annual moisture relaxes across a fixed approximately 5,760 km transport horizon. Across the three subdivision-4→5 convergence worlds this keeps mean-precipitation drift below 2.1%, runoff drift below 3.0%, soft arid-area drift below 4.4%, and biome-area total variation below 3.0% while retaining causally supported lake basins.
...
2. **Completed:** conservative hillslope diffusion, drainage-conditioned continuous valley relief, basin-aware world-space channel refinement, relief-conditioned multiscale coastal amplification, shared signed shoreline sampling, and coast-consistent categorical relief preserve canonical coast and drainage authority.
3. **Partially completed:** scale-aware delta, estuary, alluvial-fan, simple-mouth, and lake-inflow classification now consumes actual terminal sediment flux. Ocean channels and distributaries terminate on the refined rendered coast, and a first explicit coastal-plain pass lowers only quiet, low-orogen continental margins without changing coast authority. Add rift-age passive-margin inheritance and evolving sedimentary delta bodies that exchange mass with the existing sediment budget.
4. **Completed nested finite-width baseline; calibration remains:** quiet-craton, boundary-distance, and planet-size-aware interior relief adds multiple broad drainage divides without moving the coast. Canonical collision, subduction, island-arc, and suture edges are now lifted onto the nested surface mesh before physical-distance profiles create narrower cores, foothills, sedimentary forelands, and low outer flexural rises. Causal controls preserve the canonical land mask and expose source-count, core-area, and neighbor-smoothness diagnostics. Continue empirical calibration of range widths, inherited-suture decay, foreland, interior, and bathymetric profiles across accepted seeds. Judge silhouette in the illustrated atlas view and physical plausibility in the relief and heightmap views.
...
Resolved river topology remains the conservative Priority-Flood graph, but presentation centerlines no longer connect lightly displaced face centers or force one sine wave into every mesh edge. Shared junction nodes and look-ahead tangents remain, while a multiscale bend signal is integrated continuously from each node to its outlet. The dominant trunk preserves basin continuity; tributaries receive stable lineage phases at confluences so adjacent branches do not repeat the same lateral wave. Physical wavelength now grows from about 95 km on tributaries to 600 km on trunk rivers, bend amplitude is preserved in kilometres instead of shrinking with process-cell size, and each edge receives nine to twenty-one samples according to its actual phase advance. Across the three large-world EPOCH references at subdivision 5, straight-path neighboring-channel alignment of 0.886-0.892 falls to 0.808-0.818, mean centerline sinuosity rises from 1.0 to 1.081-1.086, and mean bend amplitude remains a restrained 20-22 km. On the matching 4K, tectonic-subdivision-5 to surface-subdivision-6 EPOCH-11 checkpoint, alignment falls from 0.885 to 0.810, mean sinuosity rises from 1.019 to 1.060, and bend amplitude rises from 16.7 to 22.1 km. Every internal sample remains on canonical land, and receiver IDs, lake membership, mouths, annual runoff, and sediment budgets are unchanged. A terrain-aware receiver-reselection prototype was rejected: even when constrained to the same ocean outlet, it removed resolved reaches without improving the visual alignment gate. True multiple-flow or sub-cell erosion therefore remains a future core branch rather than speculative complexity in this milestone.
...
The matching 4K atlas-style surface resolves 5,272 river segments, seven lake bodies, and 99 continental relief centers with zero canonical coast or drainage-anchor mismatches. Runoff, hillslope transfer, seasonal lake water, and sediment accounting close to floating-point residuals. Its original 0.886 neighboring-channel alignment motivated the branch-aware channel milestone above; the same retained geography now measures 0.810, making coastline morphology the next immediate visual-realism branch while multiple-flow routing remains a longer-term hydrology architecture option.
...
The next realism sequence is river presentation closure, coastline morphology, physical relief/heightmap calibration, climate calibration, and then another continent-complexity pass. This ordering is intentional: resolved coasts and relief are upstream inputs to moisture transport, rain shadows, erosion, and biome boundaries, while final continent-complexity tuning should be judged through the improved terrain rather than through provisional flat fills. Dynamic plate lifecycle remains the long-history architectural horizon and should interrupt this sequence only if the calibrated 120 Myr production window becomes a blocker.
