...
**Status: annual physical-atlas, hierarchical-drainage, and continuous-hydrography milestone implemented; physical lake balance and orogenic calibration underway.** Reduced spherical prevailing winds, physical-distance-scaled moisture advection, orographic loss, land distance-to-ocean, continentality, seasonal-temperature range, PET-relative aridity, runoff closure, hierarchical Priority-Flood routing, bounded spill-basin lake cover, river hierarchy, stream-power incision, lithology resistance, biome classification, and closed sediment/area budgets exist. Atmospheric pass count, recharge, precipitation loss, recycling, and uplift response are expressed against physical edge distance instead of process-cell count. A deterministic one-level-coarser process world owns inland basin succession; fine descendants may refine local channels and coastal outlets but every inland cross-parent receiver must inherit its anchor receiver. Renderer-only river nodes are shared at confluences, smoothed, and kept within a bounded fraction of their process cells. Lake presentation uses a continuous radial-basis reconstruction of each resolved Priority-Flood depth contour, with canonical cells retaining all area and water accounting. Surface relief distinguishes narrow continental-collision cores, landward subduction ranges, oceanic island arcs, inherited sutures, and broader foothills. Unsupported high elevation is compressed without changing canonical land authority, and an orogeny diagnostic exposes the geological support behind every rendered range. Seasonal pressure/circulation, hillslope diffusion, explicit lake evaporation/overflow balance, dynamic deltas, and empirical biome calibration remain open.
...
1. Add explicit annual lake water balance, evaporation, overflow, and calibrated connected shore contours instead of interpreting Priority-Flood capacity as lake volume; retain the completed shared sub-cell river geometry and continuous depression-contour presentation.
2. Add scale-aware delta/fan geometry and a bounded local channel refinement pass that preserves the inherited receiver graph and discharge at every junction.
