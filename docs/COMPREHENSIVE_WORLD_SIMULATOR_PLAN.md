...
**Status: first-order implementation complete, calibration ongoing.** Crustal elevation, age-dependent bathymetry, one area-weighted sea level, shelves, flooded land authority, geology-conditioned relief, and broad continental-interior relief provinces exist. Interior centers are selected from quiet, old, boundary-distant crust and evaluated at fixed physical wavelengths, so increasing planet radius can create more independent divides rather than stretching the same slope field. Flexure, dynamic topography, foreland-basin subsidence, and empirical hypsometry calibration remain open.
...
3. **Partially completed:** scale-aware delta, estuary, alluvial-fan, simple-mouth, and lake-inflow classification plus presentation distributaries now consume the completed mouth interface. Add evolving sedimentary delta bodies and explicit coastal plains while preserving discharge and sediment budgets at every junction.
4. **Partially completed:** quiet-craton, boundary-distance, and planet-size-aware interior relief now adds multiple broad drainage divides without moving the coast. Continue improving canonical elevation and bathymetry with flexure and foreland-basin subsidence, then calibrate collision, subduction, island-arc, suture, and interior profiles across accepted seeds. Judge silhouette in the illustrated atlas view and physical plausibility in the relief view.
