...
- Continental material is initialized by unequal plate-aware graph growth with multiple offset lobes. The number of emerged landmasses results from crust history and flooding rather than a final continent-count control.
...
- The resolution-independent surface sampler exposes one canonical world through synchronized natural, heightmap, climate, biomes, precipitation, aridity, temperature, continentality, drainage, wind, lithology, and orogeny modes. Presentation resolution and style do not reclassify the coast.
...
**Status: annual physical-atlas, hierarchical-drainage, continuous-hydrography, and equilibrium-lake baseline implemented; depression evolution is the next architectural decision.** Reduced spherical prevailing winds, physical-distance-scaled moisture advection, orographic loss, land distance-to-ocean, continentality, seasonal-temperature range, PET-relative aridity, runoff closure, hierarchical Priority-Flood routing, river hierarchy, stream-power incision, lithology resistance, biome classification, and closed sediment/area budgets exist. Atmospheric pass count, recharge, precipitation loss, recycling, and uplift response are expressed against physical edge distance instead of process-cell count. A deterministic one-level-coarser process world owns inland basin succession and annual lake level; fine descendants inherit its lake area and threshold while refining presentation and local channels. Closed lakes expand down their resolved depression contour until open-water evaporation balances routed inflow; overflowing lakes consume their evaporation capacity and preserve residual discharge. Total runoff closes against ocean outflow plus actual lake evaporation. Renderer-only river nodes are shared at confluences, internal lake-routing segments are hidden, and explicit ocean-mouth/lake-inflow records provide a stable delta interface. Lake presentation uses a continuous radial-basis reconstruction of each solved Priority-Flood depth contour. Surface relief distinguishes narrow continental-collision cores, landward subduction ranges, oceanic island arcs, inherited sutures, and broader foothills. Seasonal pressure/circulation, hillslope diffusion, depression breaching, dynamic deltas, and empirical biome calibration remain open.
...
1. Decide and implement depression evolution: preserve all Priority-Flood spill basins, or add a hybrid fill-or-breach geomorphic pass that can incise persistent spillways while retaining tectonic/endorheic lakes.
2. Add scale-aware delta/fan geometry from the completed ocean-mouth/lake-inflow interface and a bounded local channel refinement pass that preserves discharge at every junction.
3. Improve canonical surface elevation, bathymetry, coast antialiasing, and drainage density without changing macro-land authority; judge silhouette in the illustrated atlas view and physical plausibility in the relief view.
4. Calibrate the implemented collision/subduction/island-arc/suture profiles across accepted seeds, then replace remaining process-grid facets with convergent multiscale local-relief shading.
5. Calibrate the implemented land distance-to-ocean, continentality, aridity, and biome fields against Earth-derived ranges and diverse accepted seeds without allowing classification to mutate climate or coast state.
6. Retain one simulated surface in a worker and switch diagnostic modes, atlas/globe projection, and style without regeneration; progressively rasterize 4K–8K tiles from that retained state.
7. Calibrate whole-world composition and resolved detail against Earth epochs, synthetic fixtures, the Malazan-inspired private aesthetic targets, and the catalogued external simulator references.
...
### Architectural decision: permanent spill basins versus geomorphic breaching

The current baseline is internally conservative and resolution-stable, but generated relief still contains more deep closed depressions than mature terrestrial landscapes. Keeping every resolved Priority-Flood depression produces occasional large inland seas. A hybrid fill-or-breach model would compare evaporation-supported lake persistence against the lithology- and discharge-dependent cost of cutting a spillway, then lower a bounded path and rerun routing/erosion. This is the recommended realism path because it lets tectonic, glacial, volcanic, and arid endorheic basins survive while allowing powerful rivers to remove numerical or geologically weak dams. It is an architectural change: elevation and receiver topology would become co-evolved geomorphic state rather than immutable inputs to one erosion pass. The simpler alternative is to retain permanent spill basins and only calibrate lake thresholds, which preserves the present pipeline but leaves large-lake frequency as a tunable artifact.
