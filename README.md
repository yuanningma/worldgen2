...
- deformation-guided production terranes whose moderate tangent-plane strain grain, sparse folded resistant belts, and bounded reach create non-convex accretion fronts without turning plate motion into long ribbon continents; the field activates only at subdivision 4+, where both the belt and adjacent ocean can be resolved;
- budget-aware regional placement for the supplemental accreted terranes available on larger planets: small blocks remain attached near their nucleus, larger blocks may shape a cape or embayment, and primary Earth-scale assemblies retain their proven topology and random stream;
...
- boundary-specific orogenic profiles lifted from canonical tectonic edges onto the nested process mesh, producing physically scaled collision cores, landward subduction ranges, oceanic island arcs, inherited sutures, lower foothills, foreland basins, and outer bulges without face-sized mountain sheets;
...
- reduced spherical atmospheric circulation with latitude-dependent wind belts, a resolution-stable fixed-physical moisture horizon, physical-distance-scaled ocean recharge and downwind transport, orographic precipitation, and rain shadows;
...
- a separate three-direction, domain-warped 14–230 km presentation spectrum that adds heightmap detail without directional hatching; its finest octaves remain unshaded until an output-scale tiled normal-map pass is available;
...
- relief-conditioned world-space channel paths with shared junction tangents, trunk continuity, and stable tributary-lineage phases that add resolved lowland bends without making every branch repeat the same curve; steep mountain channels remain confined and all junctions stay tied to the resolved drainage graph;
